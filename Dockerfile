...
RUN mkdir /gen_unix_domain_socket/
COPY gen_unix_domain_socket /gen_unix_domain_socket/
COPY setup.py /
COPY README.md /
RUN find /gen_unix_domain_socket/ -name "*.editorconfig" -type f -exec rm -Rf {} \;
...
RUN rm -rf /gen_unix_domain_socket/
RUN rm -f setup.py
RUN rm -f README.md
